...
class FloatingModule(reactContext: ReactApplicationContext) :
    ...
    @ReactMethod
    fun startFloating(apiKey: String, mode: String, backendUrl: String, deviceId: String, authToken: String) {
        ...
        val intent = Intent(ctx, FloatingService::class.java).apply {
            ...
            putExtra(FloatingService.EXTRA_AUTH_TOKEN, authToken)
        }
    ...
    fun setMode(mode: String) {
        val ctx = reactApplicationContext
        val intent = Intent(ctx, FloatingService::class.java).apply {
            action = FloatingService.ACTION_UPDATE_MODE
            putExtra(FloatingService.EXTRA_MODE, mode)
        }
        ctx.startService(intent)
    }
    ...
    fun isAccessibilityEnabled(promise: Promise) {
        promise.resolve(BolkarAccessibilityService.isEnabled())
    }
